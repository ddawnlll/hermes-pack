...
```
   DIVERGENCE  ◄─────────────── ORCHESTRATOR ──────────────►  CONVERGENCE
   (feeds hypotheses)              (dispatch, no longer          (meta-gate, vetoes)
                                     the sole merger)

   EXPLORER                                                     RED TEAM
   idle-triggered, no veto                                       conditional, falsifiable
   protected curiosity budget                                    objections only, scar-tissue memory
```
